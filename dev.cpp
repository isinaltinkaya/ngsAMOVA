...
void DEV_input_VCF(argStruct *args, paramStruct *pars, formulaStruct *formulaSt, IO::outFilesStruct *outSt)
{
	...
	DEV_prepare_distanceMatrix_originalData(args, pars, dMS[0], vcfd, pairSt, formulaSt, outSt, sampleSt);
...
void DEV_prepare_distanceMatrix_originalData(argStruct *args, paramStruct *pars, distanceMatrixStruct *dMS_orig, vcfData *vcfd, pairStruct **pairSt, formulaStruct *formulaSt, IO::outFilesStruct *outSt, sampleStruct *sampleSt)
{
	...
	case 1:
	{
		...
		DEV_spawnThreads_pairEM_GL(args, pars, pairSt, vcfd, outSt, dMS_orig);
	...
	case 2:
	{
		...
		for (int pidx = 0; pidx < pars->nIndCmb; pidx++)
		{
			...
			// IO::print::Sfs("gt", outSt->out_sfs_fs, args, vcfd->JointGenoCountDistGT[pidx], snSites, vcfd->hdr->samples[pars->lut_idxToInds[pidx][0]], vcfd->hdr->samples[pars->lut_idxToInds[pidx][1]]);
		}
