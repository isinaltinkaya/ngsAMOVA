...
void nj_run(nj_t* nj) {
    ...
    // ensure nTreenodes within range
    ASSERT(nj->nTreeNodes > 0);

    const int nTreeIterations = nj->L - 2; // number of neighbor joining iterations needed to build the tree
    double* TotalDistances = (double*)malloc(nj->nTreeNodes * sizeof(double));
    ASSERT(TotalDistances != NULL);
    double* NetDivergence = (double*)malloc(nj->nTreeNodes * sizeof(double));
    ASSERT(NetDivergence != NULL);
    double* AdjustedDistances = (double*)malloc(nj->nTreeNodePairs * sizeof(double));
    ASSERT(AdjustedDistances != NULL);
