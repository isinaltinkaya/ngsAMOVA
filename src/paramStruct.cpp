...
static alleles_t* alleles_read(const char* fn) {
    ...
    for (i = 0, str1 = fc; ; i++, str1 = NULL) {
        ...
        tmp_baseint1 = acgt_charToInt[(int)*col];
        if (tmp_baseint1 < 0 || tmp_baseint1>3) {
        ...
        tmp_baseint2 = acgt_charToInt[(int)*col];
        if (tmp_baseint2 < 0 || tmp_baseint2>3) {
...
static void alleles_print(kstring_t* kstr, alleles_t* alleles) {
...
//static void alleles_get(alleles_t* alleles, const size_t queryposidx, char** retcontig, size_t* retpos, char* a1, char* a2) {
...
static int alleles_get(alleles_t* alleles, const char* querychr, const size_t querypos, char* a1, char* a2) {
    size_t cidx = 0;
    int baseint1;
    int baseint2;
    uint64_t tmp; // tmp storage for the current 64bit block
...
void test_alleles_t(void) {
    ...
    // -> test input (tests/data/test_alleles_t.tsv)
    // chr22	16054140	T	C
    // chr22	16054249	T	C
    // chr23	1	T	C
    // chr23	3	T	G
    ...
    alleles_t* alleles = alleles_read("tests/data/test_alleles_t.tsv");


    // -> [test1] read and retrieve the full alleles file data

    kstring_t kstr = KS_INIT;
    alleles_print(&kstr, alleles);

    // read full file string 
    FILE* fp = fopen("tests/data/test_alleles_t.tsv", "r");
...
paramStruct* paramStruct_init(void) {
    ...
    pars->nRuns = (args->nBootstraps > 0) ? (args->nBootstraps + 1) : 1;
