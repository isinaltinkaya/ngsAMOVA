...
amova_t* amova_get(dmat_t* dmat, metadata_t* metadata) {
    ...
    const uint32_t amova_needs_transform = DMAT_TRANSFORM_SQUARE;
    ...
    const int maxnThreads = args->nThreads;
    ...
    double** dmat_matrix_amova = NULL;
    if (amova_needs_transform == dmat->transform) {
        // ok; we can directly use the dmat matrix
        dmat_matrix_amova = dmat->matrix;
    } else {
        if (DMAT_TRANSFORM_NONE == dmat->transform && DMAT_TRANSFORM_SQUARE == amova_needs_transform) {
            dmat_matrix_amova = (double**)malloc(nRuns * sizeof(double*));
            ASSERT(dmat_matrix_amova != NULL);
            for (size_t i = 0;i < (size_t) nRuns;++i) {
                dmat_matrix_amova[i] = (double*)malloc(dmat->size * sizeof(double));
                ASSERT(dmat_matrix_amova[i] != NULL);
                for (size_t j = 0;j < dmat->size;++j) {
                    dmat_matrix_amova[i][j] = SQUARE(dmat->matrix[i][j]);
                }
            }
            isTransformed = true;
        } else {
    ...
    for (size_t runidx = 0;runidx < (size_t) nRuns;++runidx) {
        ...
        // -> private data
        // read-only //
        data[runidx].matrix = dmat_matrix_amova[runidx];
    ...
    if (isTransformed) {
        for (size_t i = 0;i < dmat->n;++i) {
            FREE(dmat_matrix_amova[i]);
        }
        FREE(dmat_matrix_amova);
    }
